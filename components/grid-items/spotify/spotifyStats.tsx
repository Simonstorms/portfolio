...
export default function SpotifyStats({
  ...
  const songName =
    longSongName?.length > 27
      ...
      : longSongName;
  const songArtists =
    props.artist && props.artist!.length > 30
      ? props.artist!.substring(0, 27) + "..."
      : props.artist!;
    ...
    return (
      <div>
        ...
        <span className="leading-6  text-gray-400">
          <Link href={randomSong.artistUrl}>{randomSong.artist}</Link>
        </span>
  ...
  return (
    ...
    >
      <div className=" flex flex-col">
        {!props.nowPlaying || props.artist === "Unknown" ? (
          <LastPlayed />
        ) : (
          <NowPlaying />
        )}
